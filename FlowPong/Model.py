...
class Model(object):
    ...
    def __init__(self, scope, optim=None, action_classes=2, criterion=tf.losses.softmax_cross_entropy, batch_size=128):
        self.scope = scope
    ...
    def train(self, optim, criterion):
        grads, lvs = zip(*optim.compute_gradients(self.loss(criterion), var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=self.scope)))
        ...
        train_step = optim.apply_gradients([(grads_accum[i], gv) for i, gv in enumerate(gvs)])
